...
const StudyScreenTabBar = props =>  {
  ...
  const color = position.interpolate({
    inputRange: [0, 1, 2],
    outputRange: [globalStyles.colors.secondary, globalStyles.colors.primary, globalStyles.colors.primary]
  });
  ...
  return (
    <Animated.View style={{ ...styles.tabBarContainer, backgroundColor: color }}>
      <TabBar
        {...props}
        style={{...styles.tabBar, backgroundColor: 'transparent'}}
        indicatorStyle={{ display: 'none' }}
        renderLabel={(props) => renderLabel({...props, style: styles.labelStyle})}
